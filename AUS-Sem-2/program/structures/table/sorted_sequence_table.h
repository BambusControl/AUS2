...
namespace structures
{
	...
	public:
		...
		SortedSequenceTable* clone() const override;
	...
	template<typename K, typename T>
	inline SortedSequenceTable<K, T>* SortedSequenceTable<K, T>::clone() const
	{
	...
	inline void SortedSequenceTable<K, T>::insert(const K & key, const T & data)
	{
		...
		if (found)
		{
			...
			throw std::logic_error("SortedSequenceTable<K, T>::insert() : Key is already present in the table.");
		}

		// Insert new item at the correct index
	...
	inline TableItem<K, T>* SortedSequenceTable<K, T>::findTableItem(const K & key) const
	{
		...
		return found ? list_->operator[](index) : nullptr;
	}
	...
	inline int SortedSequenceTable<K, T>::indexOfKey(const K & key, int indexStart, int indexEnd, bool & found) const
	{
		...
		// Calculate the middle index in range, and retrieve the key
		int indexMiddle = (indexStart + indexEnd) / 2;
		K keyMiddle = list_->operator[](indexMiddle)->getKey();

		bool eq = key == keyMiddle;
		if (eq)
		{
			// Item was found
			found = true;
		...
		{
			// Item was not found in the given range
			found = false;
			return key < keyMiddle ? indexMiddle : indexMiddle + 1;
		}

		// Item not yet found, halve the range
		bool less = key < keyMiddle;
		if (less)
		{
