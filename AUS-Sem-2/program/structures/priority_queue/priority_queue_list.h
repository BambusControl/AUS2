...
namespace structures
{
	...
	inline int PriorityQueueList<T>::indexOfPeek() const
	{
		if (list_->isEmpty())
		{
			throw std::logic_error("PriorityQueueList<T>::indexOfPeek() : Queue is empty!");
		}
		...
		for (PriorityQueueItem<T>* item : *list_)
		{
			// Nasiel sa item s lepsou prioritou
	...
	{
		return list_->operator[](indexOfPeek())->accessData();
	}
	...
	{
		return list_->operator[](indexOfPeek())->getPriority();
	}
}