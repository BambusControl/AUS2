...
namespace structures
{
	...
	{
		// Nakoniec vlozim ak je priorita aspon taka ako posledny
		...
		{
			// Ked najdem prvy prvok lepsi nez ja koncim
			int i = 0;
			while (priority < (*list_)[i]->getPriority())
			{
				i++;
			}
			list_->insert(item, i);
	...
	inline int PriorityQueueSortedArrayList<T>::indexOfPeek() const
	{
		if (list_->isEmpty())
		{
			throw std::logic_error("PriorityQueueSortedArrayList<T>::indexOfPeek() : Queue is empty!");
		}
		return list_->size() - 1;
	}
}