...
namespace structures
{
	...
	void PriorityQueueLimitedSortedArrayList<T>::push(const int priority, const T & data)
	{
		...
		{
			throw std::logic_error("PriorityQueueLimitedSortedArrayList<T>::push() : Priority queue is empty!");
		}
	...
	inline PriorityQueueItem<T>* PriorityQueueLimitedSortedArrayList<T>::pushAndRemove(const int priority, const T & data)
	{
		...
		if (size() > capacity_)
		{
			// Item will not fit into the ArrayList
			return PriorityQueueSortedArrayList<T>::list_->removeAt(0);
		}

		// Item was put into the ArrayList
		return nullptr;
	}
