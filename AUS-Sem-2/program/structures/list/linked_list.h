...
namespace structures
{
	...
	inline void LinkedList<T>::insert(const T & data, const int index)
	{
		...
		{
			DSRoutines::rangeCheckExcept(index, size_, "LinkedList<T>::insert - Index out of range!");
			LinkedListItem<T>* item = new LinkedListItem<T>(data);
	...
	{
		DSRoutines::rangeCheckExcept(index, size_, "LinkedList<T>::removeAt - Index out of bounds!");
		LinkedListItem<T>* item;

		if (index == 0)
		{
			// Odstranuje sa prvy prvok
			item = first_;
			first_ = item->getNext();

			if (item == last_)
			{
				// Prvok bol zaroven posledny
				last_ = nullptr;
			}
		}
		else if (index == size_ - 1)
		{
			// Odstranuje sa posledny prvok
			item = last_;
			last_ = getItemAtIndex(index - 1);
			last_->setNext(nullptr);
		}
		else 
		{
			// Odstranuje sa prvok v 'strede'
	...
	{
		DSRoutines::rangeCheckExcept(index, size_, "LinkedList<T>::getItemAtIndex - Index out of range!");
		LinkedListItem<T>* item = first_;
